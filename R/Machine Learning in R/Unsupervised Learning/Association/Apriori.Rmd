...
The code below coreces the dataset to a **transcation**. Once the data has been coerced to transactions it is ready for mining itemsets or rules. Association Rule Learning uses the transaction data files available in R. The transaction class is a direct extension of the ```itemMatrix``` class.
```{r, eval=TRUE, include=TRUE}
