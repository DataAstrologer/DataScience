...
Coverting the user variable to a factor.
```{r}
...
The code below coreces the dataset to a transcation. Once the data has been coerced to transactions the data is ready for mining itemsets or rules. Association Rule Learning uses the transaction data files available in R. The transaction class is a direct extension of the ```itemMatrix``` class.
```{r, eval=TRUE, include=TRUE}
...
The plot below shows item frequencies greater than 80% support. The ```itemFrequencyPlot()``` function creates an item frequency bar plot for inspecting the distribution of the objects in the transaction. This function is a part of the ```arules``` package.
```{r, eval=TRUE, include=TRUE}
