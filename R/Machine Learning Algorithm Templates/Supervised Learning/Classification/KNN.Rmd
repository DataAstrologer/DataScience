...
* The ```Forensic Glass Fragments``` dataset which is a part of the ```MASS``` package in ```R``` is used here. The dataset classifies glass fragements into 7 types. 
```{r, eval=TRUE}
library(MASS)
?fgl
```
...
are on a small scale. Thus, we are going to standardize all the independent variables (predictor) variables except the dependent variable (type). All the independent variables have a mean of 1 and standard deviation of 0 after standardization.
```{r}
...
Now, we can iterate for different values of k and decide k on the basis of value giving the lowest mean error. This process can be automated using a *for loop*. The code and the results for k value upto 7 is shown below. This can help us decide the optimal number for k. 
...
We can now plot the all the values of k with thier respective error rates. This will help us decide the best value of k. Before that, we can find out the minimum error rate and the value of k having the minimum error rate using the code below.
```{r}
