...
* The Decision Tree algorithm used **Entropy** for classification.
* The **credit.csv** dataset from the data folder is used here. The dataset contains various attributes for predicting whether a given customer would default a loan or not. The dependent variable (default) can have a value of either 1 (Non-Default) or 2 (Default).
...
The ```tree()``` function from the ```tree``` package is used for fitting the decision tree to the dataset. 
```{r}
...
The model command above prints the actual decision tree. In order to see the decisions, we call the ```summary()``` function on the model. 
```{r}
summary(model)
```
![summary_credit_tree](./data/images/summary_credit_tree.png)

Notice that the output indicates that only 6 variables have been used for constructing the tree. Also, the training MSE is 1.007. The plot of the tree is shown below.
```{r}
plot(model)
text(model, pretty = 0)
```
...
Now, we will see if the tree needs pruning. The function ```cv.tree()``` of the ```tree``` package performs cross validation in order to determine the optimal level of tree complexity. We use the argument ```FUN = prune.misclass``` in order to indicate order to indicate that we want the classification error rate to guide the cross validation and pruning process rather than the default.
```{r}
set.seed(1)
cv_tree_model = cv.tree(model, FUN = prune.misclass)
```

A plot of different tree sizes and thier residual sum of squares is shown below. A tree size with the lowest residual sum of squares is desired. 
```{r}
...
The tree with the best size is given by the command below:
```{r}
