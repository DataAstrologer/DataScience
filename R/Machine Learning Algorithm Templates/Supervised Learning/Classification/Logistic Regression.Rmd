...
The misscalssification rate of the model is given by:
```{r}
(7+4)/(54+4+7+35)
```
The misscalssification rate is 11%.
...
The ```multinom()``` function from the ```nnet``` package is used for performing multinominal logistic regression in R. The logit model is *fitted* on the training data. The ```summary``` function is then used for getting detailed information about the p-values, residual deviance and AIC the logistic model.
