...
* Logisitc Regression is a part of supervised machine learning and thus the dataset is split into **training and testing**.
...
summary(model)
```
![model_summary](./data/images/model_summary.png)
...
The ```multinom()``` function from the ```nnet``` package is used for performing multinominal logistic regression in R. The logit model is *fitted* on the training data. The ```summary``` function is then used for getting detailed information about the p-values, residual deviance and AIC for the logistic model.
