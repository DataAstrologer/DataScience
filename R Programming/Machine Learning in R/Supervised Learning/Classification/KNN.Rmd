...
* The ```Forensic Glass Fragments``` dataset which is a part of the ```MASS``` package in ```R``` is used here. The dataset classifies glass fragments into 7 types based on various attributes such as refractive index, sodium content, silicon content, etc. 
```{r, eval=TRUE}
library(MASS)
?fgl
```
